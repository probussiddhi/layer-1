...
def extract_text_from_pdf_tw(pdf_file):
    """
    Extract text from a PDF file. Handles both file paths and BytesIO objects.
    """
    if isinstance(pdf_file, str):
        pdf_document = fitz.open(pdf_file)  # Open PDF from file path
    elif isinstance(pdf_file, BytesIO):
        pdf_document = fitz.open(stream=pdf_file, filetype="pdf")  # Open PDF from BytesIO stream
...
def extract_period_of_insurance(text):
    """
    Extract the period of insurance from the text.
    """
...
def extract_policy_number(text):
    """
    Extract the policy number from the text.
    """
...
def extract_vehicle_details(pdf_file):
    ...
    """
    if isinstance(pdf_file, str):
        document = fitz.open(pdf_file)  # Open PDF from file path
    elif isinstance(pdf_file, BytesIO):
        document = fitz.open(stream=pdf_file, filetype="pdf")  # Open PDF from BytesIO stream
...
def extract_field_value(text, field_pattern):
    """
    Extract field values using a regular expression pattern.
    """
...
def extract_data_from_pdf_pcv(pdf_file):
    """
    Extract complete data from the PDF for two-wheeler insurance.
    """
    try:
        pdf_text = extract_text_from_pdf_tw(pdf_file)
